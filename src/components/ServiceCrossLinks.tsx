        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 md:gap-6">
          {others.map((svc) => (
            <Link
              key={svc.slug}
              href={`/${svc.slug}`}
              className="group bg-white rounded-2xl border border-slate-200 p-5 md:p-6 hover:border-red-300 hover:shadow-lg transition-all"
            >
              <div className="w-10 h-10 bg-red-50 rounded-xl flex items-center justify-center text-red-600 mb-3 group-hover:bg-red-600 group-hover:text-white transition-colors">
                {svc.icon}
              </div>
              ...
              <span className="inline-flex items-center gap-1 text-red-600 text-xs font-semibold group-hover:text-red-700">
